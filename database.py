...
# =================================================================
# === FUNCIÓN init_db() CON AUTO-CORRECCIÓN ===
# =================================================================
...
def init_db():
    ...
    try:
        # Revisa si la tabla 'usuarios' existe antes de intentar obtener sus columnas
        if not inspector.has_table('usuarios'):
            print("La tabla 'usuarios' no existe. Se acaba de crear. No se requiere migración.")
            return

        user_columns = {c['name'] for c in inspector.get_columns('usuarios')}
        
        # --- LÓGICA DE AUTO-CORRECCIÓN ---
        # Si las columnas '..._old' existen, se eliminan.
        # Esto limpiará la base de datos de ejecuciones de migración anteriores e incompletas.
        with engine.connect() as connection:
            ...
            if 'role_old' in user_columns:
                ...
                user_columns.remove('role_old') # Actualizar nuestro conjunto de columnas
            ...
            if 'turno_old' in user_columns:
                ...
                user_columns.remove('turno_old') # Actualizar nuestro conjunto de columnas
        
        # --- LÓGICA DE MIGRACIÓN ---
        # Si la columna 'role' (la original) todavía existe, entonces se debe ejecutar la migración.
        if 'role' in user_columns:
            print("Detectada estructura de datos antigua (columna 'role' encontrada). Se requiere migración.")
            _execute_migration() # Llama a la función de migración
        else:
...
def _execute_migration():
    """Función dedicada a migrar la estructura antigua a la nueva."""
    ...
    try:
        with engine.connect() as connection:
         ...
         print("La migración falló. La base de datos podría estar en un estado inconsistente.", file=sys.stderr)
